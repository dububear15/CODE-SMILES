...
export class StaffBookingComponent implements OnInit {
  ...
  get isPhoneValid(): boolean {
    return /^09\d{9}$/.test(this.patientPhone.trim());
  }
  ...
  get isAgeValid(): boolean {
    if (!this.patientAge.trim()) return true;
    const age = Number(this.patientAge);
  ...
  get canSubmit(): boolean {
    return (
      ...
      this.selectedSubServices.length > 0 &&
      !!this.selectedDate &&
      !!this.selectedTime
    );
  }
  ...
  generateAvailableSlots(date: string): TimeSlot[] {
    ...
    const closingMinutes = day === 0 ? 21 * 60 + 30 : day === 6 ? 21 * 60 : 20 * 60 + 30;
    const candidateStarts = [9 * 60, 10 * 60 + 30, 13 * 60, 14 * 60 + 30, 16 * 60, 17 * 60 + 30, 19 * 60];
  ...
  submit(): void {
    ...
    const staffNotes = [
      ...
      this.patientAge.trim() ? `Patient age: ${this.patientAge.trim()}` : '',
